...
__global__ void uncoalesced_naive_sgemm_kernel(const float* A, const float* B, float* C, int M, int N, int K)
{
...
void launch_uncoalesced_naive_sgemm(const float* A, const float* B, float* C, int M, int N, int K)
{
    ...
    uncoalesced_naive_sgemm_kernel<<<grid, block>>>(A, B, C, M, N, K);
}